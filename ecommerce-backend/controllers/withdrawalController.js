...
const User = require('../models/User');
const WalletTransaction = require('../models/WalletTransaction');
const { createNotification } = require('./notificationController'); // 🚀 IMPORTED NOTIFICATIONS
...
exports.requestWithdrawal = async (req, res) => {
  try {
    ...
    // 🚀 NEW: Send Notification to User that request was received
    await createNotification(
      userId,
      "Withdrawal Requested",
      `Your request to withdraw ₹${amount.toLocaleString('en-IN')} via ${method} has been received and is pending admin approval.`,
      "info",
      "/wallet"
    );
  ...
  try {
    ...
    if (status === 'approved') {
      ...
      await createNotification(
        user._id,
        "Withdrawal Approved 💸",
        `Your withdrawal of ₹${withdrawal.amount.toLocaleString('en-IN')} has been approved and transferred to your ${withdrawal.method}.`,
        "success",
        "/wallet"
      );
      ...
      await createNotification(
        user._id,
        "Withdrawal Rejected",
        `Your withdrawal of ₹${withdrawal.amount.toLocaleString('en-IN')} was rejected. The funds have been returned to your wallet balance.`,
        "cancel",
        "/wallet"
      );
    }
    ...
    // Update the user's visible transaction history!
    await WalletTransaction.findOneAndUpdate(
      { relatedOrderId: withdrawal._id }, 
      { status: status === 'approved' ? 'completed' : 'rejected' }
    );
