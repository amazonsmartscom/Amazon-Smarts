...
// 🚀 INNER COMPONENT (Reads the URL safely)
function RegisterContent() {
  const { register } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();
  
  // 🚀 THE MAGIC: Grab the referral code from the URL (e.g. ?ref=GADGET50)
  const urlReferralCode = searchParams.get('ref') || '';

  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  // 🚀 Pre-fill the state with the URL code!
  ...
  const inputStyles = "w-full p-4 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 outline-none focus:bg-white focus:ring-2 focus:ring-orange-500 transition-all shadow-sm placeholder-slate-400";
  const labelStyles = "block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1";

  return (
    <div className="min-h-screen flex font-sans selection:bg-orange-200">
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-900 text-white p-16 flex-col justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg">
          <h1 className="text-4xl font-black tracking-widest text-orange-500 mb-8">
            GADGET<span className="text-white">STORE</span>
          </h1>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-xl shadow-inner border border-slate-700">🔥</div>
              <p className="font-bold text-slate-200">Access to exclusive flash sales</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-xl shadow-inner border border-slate-700">💳</div>
              <p className="font-bold text-slate-200">Manage your affiliate wallet</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-xl shadow-inner border border-slate-700">📦</div>
              <p className="font-bold text-slate-200">Track orders in real-time</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 bg-white flex flex-col justify-center p-8 sm:p-12 md:p-16 relative">
        <div className="max-w-md w-full mx-auto">
          ...
          <div className="lg:hidden mb-10 text-center">
            <h1 className="text-3xl font-black tracking-widest text-orange-500">
          ...
          {error && (
            <div className="bg-red-50 border border-red-100 text-red-600 p-4 rounded-xl text-sm font-bold flex items-center gap-3 mb-8 shadow-sm">
              <span>⚠️</span> {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Rahul Chauhan" className={inputStyles} />
            </div>

            <div>
              <label className={labelStyles}>Email Address</label>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rahul@example.com" className={inputStyles} />
            </div>
            ...
            <div>
              ...
              <input 
                ...
                className={`${inputStyles} font-mono uppercase text-orange-600 tracking-wider`} 
              />
              {urlReferralCode && (
            ...
            <button 
              type="submit" 
              disabled={loading} 
              className={`w-full font-black uppercase tracking-widest py-4 rounded-xl shadow-lg transition-all mt-4 ${
                loading ? 'bg-slate-300 text-slate-500' : 'bg-slate-900 hover:bg-orange-500 hover:shadow-orange-500/30 hover:-translate-y-1 text-white'
              }`}
            >
          ...
          <p className="text-center mt-8 text-sm text-slate-500 font-medium">
            ...
            <Link href="/login" className="font-black text-slate-900 hover:text-orange-500 transition-colors">
              Sign in
            </Link>
          </p>
