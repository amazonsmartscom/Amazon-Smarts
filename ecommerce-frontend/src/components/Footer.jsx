    ...
    <footer className="bg-slate-900 text-slate-400 pt-16 pb-8 border-t border-slate-800 mt-auto">
      <div className="max-w-[1600px] mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
        ...
        <div>
          ...
          <div className="flex bg-slate-800 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-orange-500 transition-shadow">
            <input type="email" placeholder="Enter your email" className="bg-transparent w-full p-3 px-4 focus:outline-none text-white text-sm" />
            <button className="bg-orange-500 text-white font-black px-6 text-sm uppercase tracking-widest hover:bg-orange-600 transition-colors">Join</button>
          </div>
          <p className="text-[10px] text-slate-500 mt-3 font-bold uppercase tracking-widest">Safe & Secure 256-bit SSL Checkout</p>
        </div>

      </div>
      
      {/* 🚀 FIXED: Links now open in a new tab using target="_blank" */}
      <div className="max-w-[1600px] mx-auto px-6 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-bold uppercase tracking-widest">
